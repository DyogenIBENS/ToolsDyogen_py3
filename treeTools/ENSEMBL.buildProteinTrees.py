...
__doc__ = """
Cette version est la version améliorée par Amelie
...
arguments = utils.myTools.checkArgs( \
    ...
    [("cutoff", str, "-1"), ("defaultFamName", str, "GCUSGT%08d"), ("scoreMethod", int, [1, 2, 3]),
     ("newNodeID", int, 40000000), ("recurs", bool, False)], \
    __doc__ \
    )
...
ancutils.myProteinTree.nextNodeID = arguments["newNodeID"]
...
for (nb, tree) in enumerate(ancutils.myProteinTree.loadTree(arguments["ensemblTree"])):
    treename = ''
            ...
            if node in NodeToTest:
                ...
                if (len(tree.data[node]) != len(ChildToRemove)):
                    ...
                    if ChildeNewnode == [] or (len(ChildeNewnode) + len(ChildOk)) <= 2:
                        ...
                        NewDataTreeInfo[NodeID]['taxon_name'] = phylTree.lastCommonAncestor(
                            [NewDataTreeInfo[g]['taxon_name'] for (g, _) in tree.data[NodeID]])
                        ...
                        NewDataTreeInfo[NodeID - 1]['taxon_name'] = phylTree.lastCommonAncestor(
                            [NewDataTreeInfo[g]['taxon_name'] for (g, _) in tree.data[NodeID - 1]])
                    ...
                    NewDataTreeInfo[node]['taxon_name'] = phylTree.lastCommonAncestor(
                        [NewDataTreeInfo[g]['taxon_name'] for (g, _) in tree.data[node]])
